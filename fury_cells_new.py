...
files_dir =  listdir("/Users/ritikasingh/Downloads/simple_vessel-main/src/output")
output_xml_list = []
for names in files_dir:
    if names.endswith(".xml") & names.startswith("output"):
        output_xml_list.append(names)
        output_xml_list.sort()
print(output_xml_list)

for output in output_xml_list:
    ...
    #Save all output.xml files as png's
    window.record(showm.scene, size=(900, 768), out_path="{}.png".format(output))

#Compile all png's to generate gif
frames = []
imgs = glob.glob("*.png", root_dir="/Users/ritikasingh/Downloads/simple_vessel-main/src/output")
imgs.sort()
for i in imgs:
    new_frame = Image.open(i)
    frames.append(new_frame)

frames[0].save('output_imgs.gif', format='GIF', append_images=frames[1:], save_all=True, duration=300, loop=0)
